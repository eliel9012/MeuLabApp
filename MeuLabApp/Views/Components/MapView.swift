...
struct MapView: View {
    ...
    private var receiverAnnotation: some MapContent {
        Annotation("Receptor", coordinate: receiverLocation) {
            ReceiverMarker()
        }
    }

    private var receiverRangeRings: some MapContent {
        Group {
            if showRangeRings {
    ...
    private var userRangeRings: some MapContent {
        Group {
        ...
        VStack(alignment: .trailing, spacing: 4) {
            ...
            HStack(spacing: 4) {
                Image(systemName: "globe")
                    .font(.caption2)
                    .foregroundStyle(appState.isOpenSkyEnabled ? .blue : .secondary)
    ...
    private func centerOnAircraft(_ aircraft: Aircraft) {
...
struct AircraftAnnotations: MapContent {
