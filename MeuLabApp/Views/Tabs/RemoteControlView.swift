...
struct RemoteControlView: View {
    ...
    private var quickActions:
        [(
            type: CommandType, title: String, description: String, icon: String, color: Color,
            isDestructive: Bool, requiresTarget: Bool
        )]
    {
        [
            (
                .restartService, "Restart Serviço", "Reiniciar serviço específico",
                "arrow.clockwise", .orange, false, true
            ),
            (.clearCache, "Limpar Cache", "Limpar cache do sistema", "trash", .blue, false, false),
            (
                .runHealthCheck, "Health Check", "Verificar saúde do sistema", "heart.text.square",
                .green, false, false
            ),
            (
                .backupConfig, "Backup", "Fazer backup das configurações", "square.and.arrow.up",
                .purple, false, false
            ),
            (
                .cleanupLogs, "Limpar Logs", "Limpar logs antigos", "doc.text.magnifyingglass",
                .yellow, false, false
            ),
            (
                .updateSystem, "Atualizar", "Atualizar sistema", "arrow.down.circle", .indigo,
                false, false
            ),
        ]
    }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    RemoteCommandsScreenCache.commands = loadedCommands
    ...
    private func hydrateCommandsFromCache() {
        guard !RemoteCommandsScreenCache.commands.isEmpty else { return }
        commands = RemoteCommandsScreenCache.commands
    }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    RemoteCommandsScreenCache.commands = commands
                }
            } catch {
                await MainActor.run {
...
private enum RemoteCommandsScreenCache {
    static var commands: [RemoteCommand] = []
}
...
struct CommandDetailView: View {
    ...
    private func refreshCommand() {
        ...
        Task {
            do {
                let refreshedCommand = try await APIService.shared.fetchRemoteCommand(
                    id: command.id)
                await MainActor.run {
