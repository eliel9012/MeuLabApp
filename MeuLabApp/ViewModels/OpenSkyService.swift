...
actor OpenSkyService {
    ...
    private init() {
        let envSecret = ProcessInfo.processInfo.environment["MEULAB_OPENSKY_CLIENT_SECRET"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let plistSecret =
            (Bundle.main.object(forInfoDictionaryKey: "MEULAB_OPENSKY_CLIENT_SECRET") as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func mapToAircraft(_ state: [OpenSkyStateValue]) -> Aircraft? {
        // OpenSky states can arrive with variable field counts; we need index 13 below.
        guard state.count >= 14,
...
enum OpenSkyStateValue: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        throw DecodingError.typeMismatch(
            OpenSkyStateValue.self,
            DecodingError.Context(
                codingPath: decoder.codingPath, debugDescription: "Wrong type for OpenSkyStateValue"
            ))
    }
